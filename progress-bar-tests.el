...
(ert-deftest progress-bar-dolist-test ()
  (let ((l '()))
    (progress-bar-dolist (x (list))
        (push x l)
      (sit-for 0.2))
    (should (zerop (length l))))
  (let ((l '()))
    (progress-bar-dolist (x (list 1 2 3))
        (push x l)
      (sit-for 0.2))
    (should (equalp l (list 3 2 1)))))

(ert-deftest progress-bar-dotimes-test ()
  (let ((l '()))
    (progress-bar-dotimes (x 0)
        (push x l)
      (sit-for 0.2))
    (should (zerop (length l))))
  (let ((l '()))
    (progress-bar-dotimes (x 5)
        (push x l)
      (sit-for 0.2))
    (should (equalp l (list 4 3 2 1 0)))))

(ert-deftest progress-bar-mapc-test ()
  (let ((l '()))
    (progress-bar-mapc
     (lambda (x)
       (push x l)
